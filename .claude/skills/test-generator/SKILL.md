...
Generate a single self-contained `.html` file alongside the Markdown pair above — no external CDN dependencies at view-time, so it works fully offline once generated. (This 5-set structure is specific to the interactive HTML — the Markdown pair above stays a single paper, not 5 sets; it's a print/paper artifact where "sets" and per-section submit don't apply.)

**Every subject uses the same shared template — do not hand-roll page styling per generation.** Start from `.claude/skills/test-generator/snippets/dashboard-template.html` and follow its own leading comment exactly: replace `{{BOOTSTRAP_CSS}}` with the verbatim contents of `.claude/skills/test-generator/snippets/bootstrap.min.css` (vendored Bootstrap 5, so "no CDN" still holds — the framework is inlined into the file, not linked), then fill in the remaining `{{PLACEHOLDER}}` tokens (subject heading, chapter line, marks/time, source note, five sets' worth of section content, and the Reference Material tab's four panes — see below). Do not touch the dashboard, grading engine, tab switching, or theming — they are generic across every subject and section count and already work as-is.

The page has six top-level tabs — **Set A, Set B, Set C, Set D, Set E, and Reference Material**:

- **Set A–E** — five complete, independent tests following the exact same section structure and marks scheme as each other (per the subject's format), but each drawing its questions from a **different sub-topic slice of the same chapter(s)** in scope — e.g. for a chapter with sub-sections 7.1–7.4.1, Set A draws only from §7.1, Set B from §7.2, and so on, so that across all 5 sets every sub-topic of the chapter gets tested and none is repeated between sets. If a chapter has fewer clearly separable sub-topics than 5, split unevenly rather than repeating verbatim — every set's questions must still be genuinely distinct. Each set has its own dashboard (score ring, sections-submitted count, marks-scored tile when questions carry `data-marks`, performance-band badge), hidden until that set's first section is submitted. **Every section now has its own "Submit Section" button**, not one page-wide submit: submitting a section auto-scores its objective questions (MCQ/fill-in-blank/True-False, right/wrong with the correct answer revealed on a miss) or reveals model answers for its free-text questions, locks that section's inputs, and shows an immediate per-section result line (e.g. "You scored 6/8 in this section") — so a student gets score confidence section by section, not only after finishing the whole set. That set's dashboard updates live as more of its sections get submitted.
- **Reference Material** — chapter-wide and shared across all 5 sets (not duplicated per set), four sub-tabs always in this order: **Chapter Summary**, **Most Important Topics**, **Revision Notes**, **Key Terms**:
  ...
  - *Revision Notes* — also click-to-reveal Q&A, but distinct in kind from Most Important Topics: these are realistic "likely to be asked" practice questions drawn from the source's own worked exercises (Figure It Out / practice sets), each with a worked answer — not concept definitions, and not the same questions already used in any of the 5 sets.
...
Both tab layers (the Set A–E / Reference Material layer, and the Reference Material sub-tabs) reuse the same generic `data-tab-target` script already in the template — never add Bootstrap's JS bundle just for tabs, and never build a second tab mechanism.

Light/dark theming defaults to the browser's `prefers-color-scheme`, with a manual toggle.

Your job per generation is only to author the question markup for each set's sections and the Reference Material content, following the exact data-attribute contract documented in the template's leading comment (`data-type="mcq|tf|fib"` + `data-answer=...` for auto-scored questions, `data-type="free"` + `data-model=...` for free-response questions; optional `data-marks="N"` per question for formats that assign marks, e.g. mathematics). Wrap each test section in `<section class="card section-block" data-section="...">`, and end it with a `section-submit-btn` button plus an initially-hidden `section-result` line (see the template's leading comment for the exact markup) — that's what makes the section individually submittable and gradeable. A question with a real extracted diagram/map/figure (see "Image-based questions" above) additionally gets an `<img class="q-image" src="data:image/png;base64,...">` at the top of its card body — this is compatible with every `data-type`, it's just a visual addition, not a new question type.

**Every `data-qid` (and its inputs' `name`/`id`) must be prefixed with its set letter** — `A-a1`, `B-c3`, `E-d2`, never a bare `a1` reused across sets — because the solution-help modal looks up `data-qid` page-wide and radio-button `name` grouping is document-wide (sections aren't wrapped in separate `<form>` elements). Getting this wrong either opens the wrong question's modal or lets a click in one set silently uncheck a radio in another.

**Every question, in every section, also gets a solution-help info icon** (top-right of its card) that opens the page's shared modal — using the `{{SOLUTION_HELP_TRIGGER}}`/`{{SOLUTION_HELP_TEMPLATE}}` markup in the template's leading comment — available any time, independent of that section's submit state, so a student stuck on a question can see how to get full marks rather than only finding out after grading. A modal has no size limit, so this holds more than the three-line minimum: **Answer**, **Justification**, **Approach**, and **two similar practice questions with their own short answers**. This is required for every question across every subject and every set, not optional decoration.
...
Keep individual questions visually plain: `data-marks` drives that set's Marks Scored tile but is never rendered as a per-question badge (the section header's marks badge is the only visible marks indicator), and questions carry no per-question source citation — the page's one `{{SOURCE_NOTE}}` line already covers that for the whole page. (The plain-Markdown output mode above still cites every question individually; this simplification is specific to the interactive HTML page.)

Because scoring runs entirely client-side with no server, the correct answers necessarily live in the page's source — that's expected for this offline, self-contained format, not something to work around. If, after submitting a section, the student asks in chat to have their free-response answers evaluated by the tutor (rather than just self-compared), evaluate them following ground rules 1 and 2 above (textbook-only, simple language).
